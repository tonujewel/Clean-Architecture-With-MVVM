...
class StateRenderer extends StatelessWidget {
  ...
  Widget _getStateWidget(BuildContext context) {
    switch (stateRendererType) {
      case StateRendererType.POPUP_LOADING_STATE:
        return _getPopUpDialog(context,[_getAnimatedImage(JsonManager.loading)]);
      case StateRendererType.PUPUP_ERROR_STATE:
         return _getPopUpDialog(context,[_getAnimatedImage(JsonManager.error),_getMessage(failure.message),    _getRetryButton(AppString.ok,context),]);
      case StateRendererType.FULL_SCREEN_LOADING_STATE:
      return _getItemsInColumn([_getAnimatedImage(JsonManager.loading),_getMessage(message)]);
   
      case StateRendererType.FULL_SCREEN_EMPTY_STATE:
           return _getItemsInColumn([_getAnimatedImage(JsonManager.empty),_getMessage(message)]);
      case StateRendererType.FULL_SCREEN_ERROR_STATE:
         return _getItemsInColumn([_getAnimatedImage(JsonManager.error),
  ...
  Widget _getPopUpDialog(BuildContext context,List<Widget>childres){
    return Dialog(
      shape: RoundedRectangleBorder(borderRadius: BorderRadiusDirectional.circular(AppSize.s14)),
      ...
      child: Container(
        ...
        boxShadow:const [BoxShadow(color: Colors.black26,blurRadius: AppSize.s12,offset: Offset(AppSize.s0,AppSize.s12))]),
  ...
  Widget _getDialogContect(BuildContext context,List<Widget>children){
    return Column(mainAxisAlignment: MainAxisAlignment.center,crossAxisAlignment: CrossAxisAlignment.center,mainAxisSize: MainAxisSize.min,children: children,);
  }
  ...
  Widget _getMessage(String message) {
    return Text(message,style: getMediumTextStyle(color: ColorManager.black,fontSize: FontSize.s16),);
  }
